...
import { JP_AWARDS, type LitAward, type LitWinner } from "@/lib/jpLiterature";
...
// 单条获奖：点「简介」懒加载作者介绍（Wikidata）
function LitRow({ w }: { w: LitWinner }) {
...
function AwardPanel({ award }: { award: LitAward }) {
  return (
...
// 日本文学双奖（芥川賞 / 直木賞）
export function JapanesePrize() {
  return (
    <div>
      {JP_AWARDS.map((a) => (
        <AwardPanel key={a.id} award={a} />
      ))}
    </div>
  );
}
